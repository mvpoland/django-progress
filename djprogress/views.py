...
except ImportError:  # Django <2.0 compability
...
def overview(request):
    ...
    return render('djprogress/overview.html', context, context_instance=RequestContext(request))
...
def resolve(request, progress_id):
    ...
    progress.delete()
    messages.success(request, _('Marked progress "%s" as resolved.') % progress.name)
